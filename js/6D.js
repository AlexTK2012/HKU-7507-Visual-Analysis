...
$(document).ready(function () {
    ...
    // 模拟数据
    var data = {
        fieldNames: ['work2', 'genre', 'sumr', 'mv', 'hv'],
        values: [
            [1.00, 1.00, 1.00, 0.47, 0.90]
        ]
    };
    // 设定一些方便计算的常量
    var radius = 100,
    ...
    // 加载 网络图数据
    $(document.getElementById("btn-1")).click(function () {
        console.log("btn1");
        update6D(1);
    })

    // 加载 网络图数据
    $(document.getElementById("btn-2")).click(function () {
        console.log("btn2");
        update6D(2);
    })

    function update6D(msg) {
        // 计算雷达图表的坐标
        var areasData = [];
        var values = [];

        if (msg == "1") {
            values = {0:[0.31, 0.31, 0.17, 0.70, 0.62]};
        } else {
            values = {0:[1.00, 1.00, 1.00, 1.00, 0.47]};
        }
        for (var i = 0; i < values.length; i++) {
            var value = values[i],
                area = '',
